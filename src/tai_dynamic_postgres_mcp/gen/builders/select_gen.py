...
class SelectGen(TableGen):
    ...
    def generate_tool(self, table_info: TableInfo) -> Optional[Chunk]:
        ...
        # An all-columns-ignored table has an empty projection: its SELECT would
        # fetch no columns and its Row model would be empty, so the tool raises on
        # every call. Skip it (return None) rather than register a dead tool,
        # mirroring insert/update.
        if not included:
            return None
