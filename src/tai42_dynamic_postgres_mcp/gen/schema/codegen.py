...
# Maps a PostgreSQL scalar type name to its Python annotation. Array types are
# derived from these (``T[]`` -> ``List[T]``), and type modifiers like
# ``numeric(10,2)`` are stripped before lookup. Temporal, uuid, numeric, and
# json types map to native Python objects (not ``str``), because psycopg returns
# native objects for them and a ``str`` annotation would reject those values.
SQL_TO_PYTHON = {
    # Integer types
    "serial": "int",
    "bigserial": "int",
