    ...
    def _return_type_and_doc(self, table_info: TableInfo) -> Tuple[str, str]:
        ...
        if len(pk) == 1:
            if pk[0] not in types:
                # A primary-key column is always part of the introspected column
                # set; a missing one signals inconsistent introspection, so fail
                # loudly rather than widening the return annotation to ``Any``.
                raise ValueError(
